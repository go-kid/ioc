...
func DependencyInject(r *Injector, id string, nodes []*Node) error {
	for _, node := range nodes {
		err := injectDependency(r, id, node)
		if err != nil {
			return err
		}
	}
...
func (i *unSpecifyInterfaceInjector) Inject(r *Injector, d *Node) error {
	values := r.OnGetByInterfaceType(d.Type)
	if len(values) == 0 {
	...
	for _, m := range values {
		if _, ok := m.Interface().(defination.NamingComponent); !ok {
			val = m
			ok = true
			break
		}
...
func (s *unSpecifyInterfaceSliceInjector) Inject(r *Injector, d *Node) error {
	values := r.OnGetByInterfaceType(d.Type.Elem())
	if len(values) == 0 {
		return nil
	}
